...
// GitHub Pages처럼 하위 경로(예: /cafe/)에 배포되어도 이미지가 깨지지 않도록,
// 이 스크립트 자신의 실제 경로를 기준으로 이미지 경로를 절대 URL로 보정
(function resolveMenuImagePaths() {
  const prefix = document.currentScript.src.replace(/js\/data\.js(?:\?.*)?$/, '');
  MENU_ITEMS.forEach(menu => { menu.image = prefix + menu.image; });
})();
